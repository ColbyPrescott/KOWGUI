...
void InitGUIActivatedDemo() {
    panels.activatedDemo->AddChildren({
        ...
        (new Toggleable)->SetPosition(20, 20)->SetSize(160, 120)->AddChildren({
            ...
            (new Text)->SetY(20)->SetText("Press")->SetFontSize(20)->SetAlignments(Text::HorizontalAlign::center, Text::VerticalAlign::top, Text::FontAlign::ascender),
            ...
            (new NActivated)->SetPosition(10, 60)->SetSize(70, 50)->AddChildren({
                ...
                (new Text)->SetText("NActivated")->SetFontSize(12)->SetColor(Color::white)->SetAlignments(Text::HorizontalAlign::center, Text::VerticalAlign::middle, Text::FontAlign::middle),
            }),
            (new Activated)->SetPosition(80, 60)->SetSize(70, 50)->AddChildren({
                (new Rectangle)->SetFillColor(Color::lime),
                (new Text)->SetText("Activated")->SetFontSize(12)->SetColor(Color::black)->SetAlignments(Text::HorizontalAlign::center, Text::VerticalAlign::middle, Text::FontAlign::middle),
            }),
        }),

        // Description text
        (new Text)->SetPosition(200, 20)->SetSize(480 - sidebarWidth - 200 - 20, 200)->SetText("Feedback nodes will disable themselves and all child nodes if the closest (parent, grandparent, etc...) interactable node does not meet a specific condition.\n\nThe Activated and NActivated (Not Activated) nodes will check if the closest TOGGLEABLE node is activated or not. That is, a shortcut for whether the internal click counter is odd or even.")->SetFontSize(11)->SetVerticalAlign(Text::VerticalAlign::top)->SetOverflow(Text::Overflow::wrap)->SetWrapLineSpacing(1.1),
    });
}